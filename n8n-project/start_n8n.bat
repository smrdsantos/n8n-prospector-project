...
:exec_ignore
rem Igual ao :exec, mas não quebra o fluxo se falhar (loga como [SKIP])
setlocal
if not defined DESC set "DESC=(sem-descricao)"
call :log === %DESC% ===
>>"%LOG%" echo CMD: %*
...
rem tenta iniciar o Docker Desktop se não estiver respondendo
setlocal
...
rem decidir classe de cor por serviço
set "CLS_N8N=bad"
